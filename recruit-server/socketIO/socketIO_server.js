...
module.exports = function (server){
    ...
    io.on('connection', function (socket){
        ...
        socket.on('sendMsg', function ({targetId, myId, content}){
            console.log('Server: receive msg from client', {targetId, myId, content});
            //operate data(save msg)
            //prepare data: chat_id, create_time
            const chat_id = [targetId, myId ].sort().join('_')
            const create_time = Date.now()
            new ChatModel({from:targetId, to:myId, content, chat_id, create_time}).save(function (err, chatMsg){
                //向所有连接上的客户端发消息
